...
router.get('/:cid', async (req, res) => {
    try {
        ...
        const cart = await cartModel.findById(cid)
        if (!cart) {
            return res.status(404).json({ error: `The cart with id ${cid} does not exist` })
        }
...
router.put("/:cid", async (req, res) => {
    try {
        ...
        if (!Array.isArray(products)) {
            return res.status(400).json({ status: "error", message: 'The product array format is invalid' })
        }
...
router.delete('/:cid', async (req, res) => {
    try {
        ...
        if (!cart) {
            return res.status(404).json({ status: 'success', message: 'Invalid cart' })
        }
        res.status(200).json({ status: "success", payload: cart })
    } catch (error) {
        console.log(error)
